...
(function($, Modernizr) {
    ...
    $(document).on('ready', function() {
        var interval;
        if (Modernizr.canvas) {
            canvas = $('<canvas width="400" height="400"></canvas>').appendTo('#preload .loader').get(0);
    ...
    function draw() {
        var x = canvas.width / 2,
            y = canvas.height / 2,
            radius, width, startAngle = (currentStartAngle) * Math.PI,
            endAngle = (currentEndAngle) * Math.PI,
            offset = -90 * (Math.PI / 180);
        currentStartAngle = currentEndAngle - 0.025;
        currentEndAngle = currentEndAngle + 0.025;
        ...
        context.beginPath();
        context.arc(x, y, radius, (startAngle + offset), (endAngle + offset), false);
...
(function(w, $) {
    var $subject, subjectOffsetY, $page, $w = $(window);

    function initScroll() {
        $subject = $('nav.primary');
        subjectOffsetY = $subject.offset().top;
        $page = $page || $('body');

        function scroll() {
            var scrollTop = $w.scrollTop();
            if (scrollTop > subjectOffsetY) {
                $page.addClass('primary-navigation-fixed');
            } else {
                $page.removeClass('primary-navigation-fixed');
            }
        }
        $w.scroll(scroll);
        scroll.apply();
    ...
    $(document).on('ready', function() {
        $('body').addClass('primary-navigation-fixed');
    })
}(window, jQuery));;
/****** FILE: flavour-opener.js *****/

(function(window, $) {
    function FlavourOpener(element, options) {
        this.$element = $(element);
        this.options = $.extend(true, {
            elements: {
                container: '.flavour-facts-content',
                toggle: '.flavour-toggle-more'
            },
            closedClassName: 'flavour-closed',
            duration: 500,
            beforeToggle: $.noop,
            step: $.noop,
            easing: 'jswing',
    ...
    FlavourOpener.prototype = {
        init: function() {
            var opener = this;
            this.$element.addClass(this.options.closedClassName);
            this.isOpen = false;
            for (var key in this.options.elements) {
                this.elements['$' + key] = $(this.options.elements[key], this.$element);
            }
            if (typeof this.elements.$toggle !== 'undefined') {
                this.elements.$toggle.on('click', function(e) {
                    e.preventDefault();
                    opener.options.beforeToggle.apply(opener);
                    opener.toggle.apply(opener);
                });
            }
            this.elements.$sizer = this.elements.$container.find('> :first-child');
            this.elements.$sizer.css({
                opacity: 0
            });
        },
        toggle: function() {
            return ((this.isOpen ? this.close : this.open).apply(this));
        },
        open: function() {
            this.elements.$sizer.fadeTo(this.options.duration, 1);
            this.elements.$container.animate({
                marginTop: this.getAnimateToMaxTopPosition(),
                height: this.elements.$sizer.outerHeight()
            }, {
                duration: this.options.duration,
                step: this.options.step,
                easing: this.options.easing
            });
            this.$element.removeClass(this.options.closedClassName);
            this.isOpen = true;
        },
        close: function() {
            this.elements.$sizer.fadeTo(this.options.duration, 0);
            this.elements.$container.animate({
                marginTop: 0,
                height: 0
            }, {
        ...
        getAnimateToMaxTopPosition: function() {
            var $element = this.$element;
            var $sizer = this.elements.$sizer;
            var diff = $element.offset().top - $sizer.offset().top + 55;
            if (-diff > $sizer.outerHeight()) {
                return -($sizer.outerHeight() / 2) + this.options.offsetY;
            }
            return diff + this.options.offsetY;
            return -100;
        }
    };
    window.FlavourOpener = FlavourOpener;
    $.fn.flavourOpener = function(options) {
        return this.each(function() {
            var opener = new FlavourOpener(this, options);
            $(this).data('flavourOpener', opener);
        });
    };
    $(window).on('load', function() {
        var $elements = $('.flavour-group .flavour');
        $elements.each(function() {
            var $element = $(this);
            var $overview = $(this).find('.flavour-overview h2');
            var $subTitle = $(this).find('.flavour-facts > h3');
            var headingDiff = $overview.offset().top - $element.find('.flavour-facts-content').offset().top;
            var topReduce = headingDiff + $overview.outerHeight();
            var $p;
            $element.flavourOpener({
                beforeToggle: function() {
                    if (typeof $p === 'undefined') {
                        $p = this.$element.find('.flavour-image, .flavour-overview p');
                    }
                    $p.animate({
                ...
                step: function(val, data) {
                    var prop = data.prop;
                    if (prop !== 'marginTop') return;
                    var newval = (data.now - topReduce) - $subTitle.outerHeight();
                    $overview.css({
                        top: newval >= 0 ? 0 : newval
                    });
                    $subTitle.css({
                        top: data.now
                    });
...
(function(window, $) {
    ...
    function doScroll() {
        ...
        $watch.each(function() {
            ...
            if (goTo !== id && isScrolledIntoView('#' + id)) {
                goTo = id;
                updateNavigation(id);
                return;
            }
    ...
    $window.on('scroll', function() {
        ...
        timeout = setTimeout(doScroll, 50);
    });
    ...
    $window.on('load', function() {
        ...
        $('.toplinks a').on('click', function(e) {
            ...
            var id = href.split('#').pop().replace(/\//g, '');
            if ($('#' + id).length) {
                scrollToId(id);
                return;
            }
...
(function(window, document, undefined) {
    ...
    var applyKeyframes = function(animations, selectors) {
        ...
        for (var selectorIndex = 0; selectorIndex < selectors.length; selectorIndex++) {
            elements = queryDocument(selectors[selectorIndex][0]);
            if (!elements) {
                continue;
            }
            ...
            for (keyframeName in keyframes) {
                for (elementIndex = 0; elementIndex < elements.length; elementIndex++) {
                    ...
                    var $cur = $(curElement);
                    if ($cur.attr(attributeName)) {
                        attributeValue += $cur.attr(attributeName);
                    }
                    $(elements[elementIndex]).attr(attributeName, attributeValue);
                }
            }
        }
    };
    kickstart(queryDocument('link, style'));
}(window, document));;
/****** FILE: skrollr.js *****/
...
$(window).on('load', function() {
    if ($('body').hasClass('HomePage')) {
        var s = skrollr.init({
            ...
            forceHeight: !!$('body').hasClass('HomePage'),
            edgeStrategy: 'ease',
            easing: {
                WTF: Math.random,
                inverted: function(p) {
                ...
                stepped: function(p) {
                    var jump = 308;
                    var pPos = Math.ceil(6 * p);
                    var pVal = jump * (pPos - 1);
                    return (pVal / 1540);
                },
                round: function(p) {
                    return Math.round(p);
